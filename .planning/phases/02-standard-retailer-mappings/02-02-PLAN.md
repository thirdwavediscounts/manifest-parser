...
<task type="auto">
  <name>Task 3: Set up integration test infrastructure</name>
  <files>tests/integration/retailer-parsing.test.ts</files>
  <action>
    Create integration test file with test utilities for loading CSV and XLSX files:
  ...
  <verify>
    ```bash
    # Verify csv-parse installed and file compiles
    npx tsc --noEmit
    ```
  </verify>
  <done>Test infrastructure set up with CSV and XLSX loading utilities</done>
</task>

<task type="auto">
  <name>Task 4: Add integration tests for ACE, AMZ, ATT, BY, TGT retailers</name>
  <files>tests/integration/retailer-parsing.test.ts</files>
  <action>
    Add first batch of integration tests inside the describe block:
  ...
  <verify>
    ...
    ```
    First 5 tests should pass.
  </verify>
  <done>Integration tests pass for ACE, AMZ, ATT, BY, TGT retailers</done>
</task>

<task type="auto">
  <name>Task 5: Add integration tests for B-Stock, QVC, RC, JCP, TL retailers</name>
  <files>tests/integration/retailer-parsing.test.ts</files>
  <action>
    Add remaining integration tests including TL (XLSX format):
    ...
    Note: TL test is async due to XLSX reading. The TL retailer config uses 'Orig. Retail' as primary column for unit_retail.
  </action>
  <verify>
    ```bash
    npm test -- --testPathPattern="retailer-parsing"
    ```
    All 10 retailer tests should pass.
  </verify>
  <done>Integration tests pass for all 10 retailers including TL (XLSX)</done>
</task>
...
<success_criteria>
- All integration tests pass for sample manifests (10 tests)
...
- TL 'Orig. Retail' used for unit_retail (XLSX format verified)
