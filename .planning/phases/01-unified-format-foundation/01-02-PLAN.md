...
must_haves:
  truths:
    ...
    - "Existing raw file download path is preserved (createZipFromRawFiles remains functional)"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    Keep existing functions (createZipFromRawFiles, createZipFromManifests) unchanged.
    The raw download path must continue to work until Phase 6 adds the UI toggle.
    This plan ADDS the unified path alongside the existing raw path - it does NOT replace it.
  </action>
  <verify>
...
<task type="auto">
  ...
  <verify>
    ...
    1. chrome.runtime.sendMessage called with { type: 'PARSE_FILE', ... } pattern exists
    2. transformToUnified function is called in the processing flow
    3. generateUnifiedCsv function is called to produce CSV content
    4. createZipFromUnifiedManifests (or equivalent) used for unified output zip

    Data flow: raw base64 -> PARSE_FILE message -> ManifestItem[] -> transformToUnified -> generateUnifiedCsv -> zip
  </verify>
  <done>
    popup.ts uses unified transformation pipeline
    Processing produces unified CSV in output zip
    All key_links from must_haves are present in code
  </done>
</task>
...
7. BACKWARDS COMPATIBILITY: Existing createZipFromRawFiles function is preserved (not deleted)
   - Raw download functionality remains available for Phase 6 UI toggle implementation
